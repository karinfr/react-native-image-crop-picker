...
NSData *_Nullable tj_UIImageHEICRepresentation_picker(UIImage *const image, const CGFloat compressionQuality)
{
    ...
    if (@available(iOS 11.0, *)) {
        if (image) {
            ...
            if (destination) {
                ...
                pthread_mutex_t *lock = tj_HEICEncodingLock_picker();
                                              ...
                                              fallback:(NSData *(^)(UIImage *))fallback
{
    ...
    NSData *data = tj_UIImageHEICRepresentation_picker(image, compressionQuality);
...
BOOL tj_CGImageSourceUTIIsHEIC_picker(const CGImageSourceRef imageSource)
{
    BOOL isHEIC = NO;
    if (@available(iOS 11.0, *)) {
        if (imageSource) {
...
BOOL tj_isImageAtPathHEIC_picker(NSString *const path)
{
    BOOL isHEIC = NO;
    ...
    if (imageSource) {
        isHEIC = tj_CGImageSourceUTIIsHEIC_picker(imageSource);
        CFRelease(imageSource);
    }
    return isHEIC;
}

pthread_mutex_t *tj_HEICEncodingLock_picker(void)
{
